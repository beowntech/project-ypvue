<template>
    <div>
        <Breadcrumbs :button-enable="true" :button-title="'Add Temp Course'" :close-button="addNew" :close-title="'Close'" @action="addNew = !addNew" title="Temporary Course"/>
...
<!--        </div>-->
        ...
        <div v-else class="container-fluid">
            <div class="row">
                <div class="col-sm-3 col-xl-3" v-for="(data,i) in tempCourses" :key="i">
                    <div class="card">
                        <div class="guru-img">
                            <img :src="getCoursePath(data.image,data.id)" style="width: 100%;" alt="" class="img-fluid">
                        </div>
                        <div class="product-details p-2">
                            <div class="row">
                                <div class="col"> <h4>{{data.name}}</h4></div>
                                <div class="col-1"><i class="fa fa-pencil edit-pencil font-dark" style="cursor: pointer" @click="$router.push({path: `update/${data.id}`})"></i></div>
                                <div class="col-2 mr-2"><i class="fa fa-trash edit-pencil font-danger" style="cursor: pointer" @click="selectedTempId = data.id" v-b-modal.delete_temp></i></div>
                            </div>
        ...
        <b-modal id="delete_temp" centered
                 title="Delete Temp Course"
                 ok-title="Delete"
                 @hidden="selectedTempId=null"
                 @cancel="selectedTempId=null"
                 @close="selectedTempId=null"
                 @hide="selectedTempId=null"
                 @ok="deleteTempCourse"
                 class="theme-modal">
            <p>Are you sure want to Delete this Temporary Course?</p>
        </b-modal>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            addTempCourse() {
                ...
                axios.post(apiUrls.addTempCourse, formData, {
                    headers: {
            ...
            deleteTempCourse(){
                axios.post(apiUrls.deleteTempCourse,{
